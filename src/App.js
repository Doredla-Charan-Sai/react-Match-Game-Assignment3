...
class App extends Component {
  ...
  onTabTouch = uniqueId => {
    this.setState({isActive: uniqueId})
  }

  onImageTouch = uniqueId => {
    const getRandomImgId = Math.floor(Math.random() * imagesList.length)
    const {activeImageId} = this.state
    if (activeImageId !== uniqueId) {
      this.clearIntervalItem()
    } else {
      this.setState(prevState => ({
        activeImageId: imagesList[getRandomImgId].id,
        score:
          prevState.activeImageId === uniqueId
            ? prevState.score + 1
            : prevState.score,
      }))
    }
  ...
  render() {
    const {seconds, score, isActive, activeImageId, playing} = this.state
    console.log(activeImageId)
    const getFilteredList = imagesList.filter(
      eachItem => eachItem.category === isActive,
    )
    return (
      <div>
        <navbar>
          <ul className="nav-bar-cont">
            <li>
              <img
            ...
            <li className="score-time-cont">
              ...
              <div className="time-cont">
                <img
                  ...
                  alt="timer"
                />
                <p className="score score-value">{seconds} sec</p>
              </div>
            </li>
          </ul>
        </navbar>
        <div className="bg-cont">
          {playing ? (
            <>
              <img
                src={
                  imagesList.find(each => each.id === activeImageId).imageUrl
                }
                alt="match"
                className="main-img"
              />
              <ul className="tabs-list-cont">
                {tabsList.map(eachTab => (
                  <TabItem
                    key={eachTab.tabId}
                    tabDetails={eachTab}
                    isActiveTab={eachTab.tabId === isActive}
                    onTabClick={this.onTabTouch}
                  />
                ))}
              </ul>
            ...
            <div className="score-board">
              ...
              <h1 className="your-score get-score">{score}</h1>
              <button
                type="button"
                className="play-again your-score"
